...
(async()=>{
  try{
    ...
    grid.innerHTML=shows.map((show,index)=>`<a class="show-card" href="${escapeHtml(show.path)}" style="--accent:${escapeHtml(show.accent||'#8c7cf3')}"><div class="show-art-wrap"><img class="show-art" src="${escapeHtml(show.image)}" alt="${escapeHtml(show.name)} logo" ${index>0?'loading="lazy"':''} decoding="async"></div><div class="show-copy"><div><h3>${escapeHtml(show.name)}</h3><p>${escapeHtml(show.description||'Open production tools')}</p></div><span class="open-bubble" aria-hidden="true">→</span></div></a>`).join('');
