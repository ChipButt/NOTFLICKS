(() => {
  ...
  const CACHE_KEY = 'planuf-checklist-cache-v4';
  const LEGACY_CACHE_KEY = 'planuf-checklist-cache-v3';
  ...
  const TYPE_LABELS = {
    document: 'Document / artwork', image: 'Image', font: 'Font', palette: '5-colour palette',
  ...
  const DEFAULT_PALETTE = ['#8C7CF3', '#A395FF', '#F4F1FF', '#241C43', '#120F24'];
  ...
  function makeItem(title, type) {
    return {
      ...
      ...(type === 'palette' ? { colours: [...DEFAULT_PALETTE] } : {})
    };
  }
  ...
  function freshState() {
    ...
    return { version: 4, updatedAt: null, activeShowId: show.id, shows: [show] };
  }
  function loadLocal() {
    ...
    for (const show of state.shows) {
      ...
      for (const item of show.items) {
        ...
        if (item.type === 'palette') {
          ...
          const colours = Array.from({ length: 5 }, (_, i) => normaliseHex(incoming[i]) || DEFAULT_PALETTE[i]);
          if (JSON.stringify(colours) !== JSON.stringify(item.colours)) changed = true;
  ...
  async function requirePin() {
    ...
    return new Promise(resolve => {
      ...
      const finish = () => resolve(sessionStorage.getItem(PIN_KEY) || '');
  ...
  function render() {
    ...
    if (!show.items.length) {
      grid.innerHTML = '<div class="empty">No checklist items yet.</div>';
    } else {
      show.items.forEach(item => grid.appendChild(buildCard(show, item)));
    }
    updateStatus();
  }
  ...
  function buildCard(show, item) {
    ...
    card.innerHTML = `
      <div class="cardhead">
        <input class="check" type="checkbox" aria-label="Mark complete">
        <div><h3></h3><div class="kind"></div></div>
        <button class="x" type="button" title="Delete item">×</button>
      </div>
      <div class="content-slot"></div>
      <label class="notes"><span>Notes</span><textarea rows="3" placeholder="Add notes…"></textarea></label>`;

    card.querySelector('h3').textContent = item.title;
    card.querySelector('.kind').textContent = TYPE_LABELS[item.type];
    ...
    card.querySelector('.x').addEventListener('click', () => {
      ...
      show.items = show.items.filter(i => i.id !== item.id);
      markDirty();
      render();
    });
    ...
    if (item.type === 'palette') buildPaletteEditor(item, slot);
    else buildAssetEditor(show, item, slot);
    return card;
  }

  function buildPaletteEditor(item, slot) {
    const wrap = document.createElement('div');
    wrap.className = 'palette-editor';
    wrap.innerHTML = `
      <div class="palette-strip" aria-label="Five colour palette"></div>
      <div class="palette-fields"></div>
      <div class="assetbuttons">
        <button class="btn alt paste-colours" type="button">Paste 5 Colour Codes</button>
        <button class="btn alt reset-colours" type="button">Reset Palette</button>
      </div>`;
    ...
    const repaint = () => {
      strip.replaceChildren(...item.colours.map((hex, index) => {
        const swatch = document.createElement('div');
        swatch.className = 'palette-swatch';
        swatch.style.background = hex;
        swatch.title = `Colour ${index + 1}: ${hex}`;
        const label = document.createElement('span');
        label.textContent = hex;
        label.style.color = contrastText(hex);
        swatch.appendChild(label);
        return swatch;
      }));
    };

    item.colours.forEach((hex, index) => {
      const row = document.createElement('label');
      row.className = 'colour-field';
      ...
      const code = row.querySelector('.colour-code');
      picker.value = hex;
      code.value = hex;
      picker.addEventListener('input', () => {
        item.colours[index] = picker.value.toUpperCase();
        code.value = item.colours[index];
        repaint();
        markDirty();
      });
      code.addEventListener('change', () => {
        const value = normaliseHex(code.value);
        if (!value) { code.value = item.colours[index]; return toast('Use a hex colour such as #8C7CF3.', 'err'); }
        item.colours[index] = value;
        code.value = value;
        picker.value = value;
        repaint();
        markDirty();
    ...
    wrap.querySelector('.paste-colours').addEventListener('click', () => {
      const input = prompt('Paste five hex colour codes separated by spaces, commas or new lines.', item.colours.join(', '));
      if (!input) return;
      const found = input.split(/[\s,;|]+/).map(normaliseHex).filter(Boolean);
      if (found.length !== 5) return toast('Please enter exactly five valid hex colour codes.', 'err');
      item.colours = found;
      markDirty();
      render();
    });
    wrap.querySelector('.reset-colours').addEventListener('click', () => {
      item.colours = [...DEFAULT_PALETTE];
      markDirty();
      render();
    });
    repaint();
  ...
  function buildAssetEditor(show, item, slot) {
    ...
    wrap.innerHTML = `
      <div class="preview"><span>No file attached</span></div>
      <div class="meta"></div>
      <div class="assetbuttons">
        <button class="btn upload" type="button">Upload Shared File</button>
        <button class="btn alt link" type="button">Attach Link</button>
        <button class="btn alt clear" type="button">Remove File</button>
        <input class="picker" type="file" hidden>
      </div>`;
    slot.appendChild(wrap);
    const picker = wrap.querySelector('.picker');
    picker.accept = ACCEPT[item.type] || '*/*';
    wrap.querySelector('.upload').addEventListener('click', () => picker.click());
    picker.addEventListener('change', () => {
      const file = picker.files?.[0];
      if (file) uploadSharedFile(show, item, file);
      picker.value = '';
    });
    wrap.querySelector('.link').addEventListener('click', () => {
      const url = prompt('Paste a direct/public URL', item.file?.url || '');
      if (!url) return;
      try { new URL(url); } catch { return toast('That URL is not valid.', 'err'); }
      item.file = { name: decodeURIComponent(url.split('/').pop()?.split('?')[0] || item.title), type: guessMime(url), url, size: 0 };
      item.done = true;
      markDirty();
      render();
    });
    ...
    if (item.file) {
      wrap.querySelector('.meta').textContent = item.file.name || 'Attached file';
      renderPreview(item, wrap.querySelector('.preview'));
    }
  }
  ...
  async function uploadSharedFile(show, item, file) {
    ...
    try {
      ...
      const response = await fetch(`${API}/asset`, {
        method: 'POST', headers: { 'X-Edit-Pin': pin, 'X-State-Sha': stateSha }, body: form
      });
      if (response.status === 401) {
        sessionStorage.removeItem(PIN_KEY);
        throw new Error('Edit PIN rejected. Press Edit PIN and enter it again.');
      }
      ...
      item.file = { name: file.name, type: file.type || guessMime(file.name), size: file.size, path: result.path };
      item.done = true;
      markDirty();
      render();
      toast('File uploaded. Save Shared Changes to publish the updated checklist.');
    } catch (error) {
  ...
  function renderPreview(item, box) {
    ...
    const url = file.path ? `${API}/asset?path=${encodeURIComponent(file.path)}&type=${encodeURIComponent(file.type || '')}` : file.url;
    ...
    if (mime.startsWith('image/') || item.type === 'image') {
      const img = new Image(); img.src = url; img.alt = item.title; box.appendChild(img); return;
    }
    if (mime.startsWith('audio/') || item.type === 'audio') {
      const audio = document.createElement('audio'); audio.controls = true; audio.src = url; box.appendChild(audio); return;
    }
    if (mime.startsWith('video/') || item.type === 'video') {
      const video = document.createElement('video'); video.controls = true; video.playsInline = true; video.src = url; box.appendChild(video); return;
    }
    if (mime === 'application/pdf' || /\.pdf(?:$|\?)/i.test(url)) {
      const iframe = document.createElement('iframe'); iframe.src = url; iframe.title = item.title; box.appendChild(iframe); return;
    }
  ...
  async function renderFontPreview(url, name, box) {
    try {
      const family = `PlanufFont_${uid('f').replace(/-/g, '')}`;
      ...
      sample.textContent = currentShow().name;
      box.appendChild(sample);
    } catch {
      box.textContent = `Font attached: ${name || 'font file'}`;
    }
  }
  function clearFonts() {
    while (loadedFonts.length) {
      const font = loadedFonts.pop();
  ...
  async function loadShared(force) {
    ...
    try {
      ...
      if (payload.data?.shows) {
        state = payload.data;
        dirty = false;
        const migrated = normaliseState(true);
        if (migrated) dirty = true;
        render();
        if (migrated) toast('Colour Scheme has been added to existing shows. Save Shared Changes to publish it.');
      }
      return true;
    } catch (error) {
      updateStatus('Shared load error', 'bad');
  ...
  async function saveShared() {
    ...
    if (!pin) return;
    busy = true;
    updateStatus('Saving shared…');
    try {
      state.updatedAt = new Date().toISOString();
      const response = await fetch(`${API}/state`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'X-Edit-Pin': pin },
        body: JSON.stringify({ data: state, baseSha: stateSha })
      });
      const payload = await response.json().catch(() => ({}));
      if (response.status === 401) {
        sessionStorage.removeItem(PIN_KEY);
        $('pin').value = '';
        throw new Error('EDIT_PIN was rejected. The saved PIN has been cleared; press Edit PIN and enter the Cloudflare value again.');
      }
      ...
      stateSha = payload.sha || stateSha;
      dirty = false;
      saveLocal();
      updateStatus();
      toast('Shared checklist saved. Other devices will load this version.');
    } catch (error) {
      dirty = true;
      updateStatus('Save failed', 'bad');
      toast(error.message, 'err');
    } finally {
      busy = false;
      updateStatus();
  ...
  function guessMime(value) {
    ...
    if (/\.(mp3|wav|m4a|aac|ogg)$/.test(path)) return 'audio/mpeg';
    if (/\.(mp4|mov|webm|m4v)$/.test(path)) return 'video/mp4';
    if (/\.(ttf|otf|woff|woff2)$/.test(path)) return 'font/woff2';
